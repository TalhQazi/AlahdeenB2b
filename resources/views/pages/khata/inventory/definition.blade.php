...
@section('page')
    ...
    <div class="card col-span-2 xl:col-span-1">
        <div class="card-header">{{ __('Inventory Products Definition') }}</div>
            ...
            <table class="table-auto border-collapse w-full mt-5">
                <thead>
                    <tr class="rounded-lg text-sm font-medium text-gray-700 text-left" style="font-size: 0.9674rem">
                        ...
                        <th class="px-4 py-2 " style="background-color:#f8f8f8">{{ __('Purchase Conversion Factor') }}</th>
                ...
                <tbody class="text-sm font-normal text-gray-700" id="search_results">
                    @if (count($product_definitions) > 0)
                        @foreach ($product_definitions as $key => $definition)
                                ...
                                <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                                    ...
                                    <td class="px-4 py-4 controls">
                                        <form action="{{ route('khata.inventory.product.definition.destroy', ['product_definition' => $definition->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method("DELETE")
                                            <span>
                                                <a href="{{route('khata.inventory.product.definition.edit', ['product_definition' => $definition->id])}}" title="{{ __('Edit Product Definition') }}">
    ...
    @push('scripts')
        <script src="{{ asset(('/js/pages/definition_index.js')) }}"></script>
    @endpush

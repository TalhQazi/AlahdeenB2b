    ...
    <div class="pt-2 font-mono">
        <div class="container mx-auto">
            <div class="inputs w-full p-6 mx-auto">
                ...
                <form id="category_create_form" class="mt-6 border-t border-gray-400 pt-4" method="POST" enctype="multipart/form-data" action="{{url('/admin/category/store')}}">
                    @csrf
                    <div class='flex flex-wrap -mx-3 mb-6'>
                            ...
                            <div class="flex-shrink w-full inline-block relative">
                                <select class="block appearance-none text-gray-600 w-full bg-white border border-gray-400 shadow-inner px-4 py-2 pr-8 rounded" name="parent_cat_id">
                                    ...
                                    @foreach ($categories as $category)
                                        <option <?php echo old('parent_cat_id') == $category->id ? "selected" : "" ?> value="{{$category->id}}">{{$category->title}}</option>
                        ...
                        <div class="personal w-full border-t border-gray-400 pt-4">
                            <div class="flex justify-end">
                                <button class="border border-gray-700 bg-gray-700 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-gray-800 focus:outline-none focus:shadow-outline" type="submit">Save Category</button>
                            </div>
                        </div>
                    </div>
                </form>

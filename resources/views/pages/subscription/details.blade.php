...
@section('page')
    ...
    <div class="card col-span-2 xl:col-span-1">
        <div class="card-header">{{ __('Subscriptions / Packages') }}</div>
        <div class="bg-white pb-4 px-4 rounded-md w-full">
                ...
                <div class="mt-5 clearfix inline-block w-full">
                    <form action="{{ route('subscription.proceedPayment') }}" method="post">
                        @csrf
                        <label class="block mb-1" for="additional_info">{{ __('Additional Information (if Any)') }}</label>
                        <div class="w-full">
                            <textarea name="additional_info" id="additional_info" class="h-16 px-3 py-2 text-base text-gray-700 placeholder-gray-600 border rounded-lg focus:shadow-outline" cols="30" rows="5">{{ old('additional_info') }}</textarea>
                        </div>

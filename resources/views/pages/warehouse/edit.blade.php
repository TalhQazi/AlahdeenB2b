    ...
    <div class="min-h-screen pt-2 font-mono">
        <div class="container mx-auto">
            <div class="inputs w-full p-6">
                <h2 class="text-2xl text-gray-900">{{__('Edit Warehouse')}}</h2>
                <form id="add_warehouse_form" class="mt-6 border-t border-gray-400 pt-4" enctype="multipart/form-data" method="POST" action="{{route('warehouse.update', ['warehouse' => $warehouse->id])}}">
                    @csrf
                    @method('PUT')
                    @include('components.warehouse.basic-info')
                    @include('components.warehouse.location-infoo')

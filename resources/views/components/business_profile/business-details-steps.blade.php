...
<div class="mx-4 p-4">
    <div class="flex items-center">
        ...
        <div class="flex-auto border-t-2 transition duration-500 ease-in-out {{ Route::current()->getName() == 'profile.business.create.director.profile' ? 'border-teal-600' : 'border-gray-300' }}"></div>
        <a href="{{ route('profile.business.create.director.profile') }}" class="flex items-center {{ Route::current()->getName() == 'profile.business.create.director.profile' ? 'text-white' : 'text-gray-500'}} relative">
            <div class="rounded-full transition duration-500 ease-in-out h-12 w-12 py-3 border-2 {{ Route::current()->getName() == 'profile.business.create.director.profile' ? 'bg-teal-600 border-teal-600' : 'border-gray-300' }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-mail ">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                    <polyline points="22,6 12,13 2,6"></polyline>
                </svg>
            </div>
        <div class="absolute top-0 -ml-10 text-center mt-16 w-32 text-xs font-medium uppercase text-gray-500">{{ __('Director Profile') }}</div>
        </a>

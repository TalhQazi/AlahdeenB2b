@if (!empty($products))
    @foreach ($products->data as $key => $product)
        <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
            ...
            <td class="px-4 py-4">
                <?php
                    $showRoute = route('product.show',['product' => $product->id]);
                    $showTitle = __('View Product Details');
                    $editRoute = route('product.edit',['product' => $product->id]);
                    $editTitle = __('Edit Product');
                    if($product->deleted_at) {
                        $showRoute = route('product.show-deleted',['product_id' => $product->id]);
                        $editRoute = "#";
                        $editTitle = __('Reactivate product to edit it');
                    }
                ?>

                @if (!empty($product->promotion_offer))
                    <a href="#" id="view_promotional_button" data-promotion-id="{{$product->promotion_offer->id}}" title="{{ __('View Promotion')}}">
                        <i class="fas fa-tag"></i>
                    </a>
                @else
                    <a href="#" id="add_promotional_button" data-product-id="{{$product->id}}" title="{{ __('Add Promotion')}}">

<div class="flex items-center justify-center fixed left-0 bottom-0 w-full h-full bg-gray-800 bg-opacity-75 z-10 hidden" id="add-product-images-modal">
    <div class="bg-white rounded-lg w-1/2 sm:w-3/4">
      <div class="flex flex-col items-start p-4">
        <div class="flex items-center w-full border-b-1">
          <div class="text-gray-900 font-medium text-lg" id="modal-title">{{__('Product') }}</div>
        ...
        <hr>
        <div class="mt-6">{{ __('Add Images') }}</div>
        <div class="w-full mt-6">
            <form id="add-product-image-form" enctype="multipart/form-data" method="POST" action="{{route('product.update',['product' => $product->id])}}"  class="w-full">
                @csrf
                {{ method_field('PUT') }}

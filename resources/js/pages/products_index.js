...
function populateTable(response, displayDiv) {
  ...
  $(response.products.data).each(function(key, product) {
    ...
    if(product.deleted_at == null) {
      restoreDelete = `<a href="${base_url+'/product/delete/'+product.id}" title="Deactivate Product" class="ml-1 restore-product">
    ...
    html += `<tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
              ...
              <td class="px-4 py-4 ${product.deleted_at != null ? 'text-red-500': 'text-green-500'}"> ${product.deleted_at ? 'Deleted': 'On'}</td>
              <td class="px-4 py-4">${product.category.title}</td>
              <td class="px-4 py-4">${product.created_at}</td>
              <td class="px-4 py-4">

                  <a href="${base_url+'/product/show/'+product.id}" title="View Product Details">
                      <i class="fa fa-eye"></i>
                  </a>
                  <a href="${base_url+'/product/edit/'+product.id}" title="Edit Product" class="mx-0.5">
